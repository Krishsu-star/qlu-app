...
router.post("/login", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT u.*, e.name AS employee_name, e.employment_status FROM users u
    ...
    // Linked employees who've been separated (a Separation Date was set) can't sign in,
    // even with a correct password — accounts with no linked employee (e.g. a
    // standalone QA login) are unaffected.
    if (user.employee_id && user.employment_status === "Inactive") {
