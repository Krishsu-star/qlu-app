...
router.post("/bulk-import", requireAuth, requireRole("Admin", "HR"), async (req, res) => {
  ...
  for (const r of rows) {
    if (!r.name) continue;
    await pool.query(`INSERT INTO skills (id, name, category, department, designation, criticality) VALUES (?,?,?,?,?,?)`, [uuidv4(), r.name, r.category || "Technical", r.department || null, r.designation || null, r.criticality || "Normal"]);
    imported++;
  }
  res.json({ imported });
});
