...
function rowToSkill(r) {
  return { id: r.id, name: r.name, category: r.category, department: r.department };
}
...
router.post("/", requireAuth, requireRole("Admin", "HR", "Manager"), async (req, res) => {
  ...
  const id = uuidv4();
  await pool.query(`INSERT INTO skills (id, name, category, department) VALUES (?,?,?,?)`, [id, name, category, department]);
...
router.put("/:id", requireAuth, requireRole("Admin", "HR", "Manager"), async (req, res) => {
  const { name, category, department } = req.body;
  ...
  if (req.user.role === "Manager") {
    ...
    await pool.query(`UPDATE skills SET name=?, category=? WHERE id=?`, [name, category, req.params.id]);
    return res.json({ ok: true });
  }

  await pool.query(`UPDATE skills SET name=?, category=?, department=? WHERE id=?`, [name, category, department || null, req.params.id]);
  res.json({ ok: true });
});
...
router.post("/bulk-import", requireAuth, requireRole("Admin", "HR"), async (req, res) => {
  ...
  for (const r of rows) {
    if (!r.name) continue;
    await pool.query(`INSERT INTO skills (id, name, category, department) VALUES (?,?,?,?)`, [uuidv4(), r.name, r.category || "Technical", r.department || null]);
    imported++;
  }
  res.json({ imported });
});
